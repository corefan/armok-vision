		...
		Pass
		{
			...
			CGPROGRAM
					...
					)
				{
                    ...
                    float4 vShapeTexel = tex2D(g_tShapeMap, i.vTextureCoords.xy);
					...
					#if ( _NORMALMAP )
					{
						vNormalTs.xyz = UnpackScaleNormal( tex2D( g_tShapeMap, i.vTextureCoords.xy ).ggga, g_flBumpScale );
					...
					#if ( !S_UNLIT )
					{
						...
						#if ( S_OCCLUSION )
						{
							float flOcclusion = tex2D( g_tShapeMap, i.vTextureCoords.xy ).r;
